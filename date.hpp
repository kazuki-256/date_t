...
constexpr char DEFAULT_DATE_FORMAT[] = "%Y-%m-%d %H:%M:%S";
char DEFAULT_DATE_BUFFER[257];
constexpr size_t DEFAULT_DATE_BUFFER_LENGTH = 256;



// parse month
size_t parse_month(const char* strmonth, size_t n, int* month) {
    static constexpr char* MONTHS[12] = {"January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"};
...
size_t parse_weekday(const char* strweek, size_t n, int* week) {
    static constexpr char* WEEKDAYS[7] = {"Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"};
...
public:
    ...
    const char* c_str(char* buffer = DEFAULT_DATE_BUFFER, size_t buffer_size = DEFAULT_DATE_BUFFER_LENGTH) const {
        snprintf(buffer, buffer_size, "interval_t(%d years, %d months, %d days, %d hours, %d mintues, %d seconds)",
...
public:
    ...
    date_t(const char* strdate, const char* fmt = DEFAULT_DATE_FORMAT) {
        parse_date(strdate, fmt);
    }

    date_t(std::string& strdate, const char* fmt = DEFAULT_DATE_FORMAT) {
        parse_date(strdate.c_str(), fmt);
    }


    date_t& parse_date(const char* strdate, const char* fmt = DEFAULT_DATE_FORMAT) {
        struct tm tm{};
    ...
    date_t& operator=(const char* strdate) {
        struct tm tm{};
        parse_time(strdate, DEFAULT_DATE_FORMAT, &tm);
        t = mktime(&tm);
        return *this;
    }
    ...
    operator std::string() const {
        ...
        std::string s;
        s.resize(129);
        strftime(s.data(), s.capacity(), DEFAULT_DATE_FORMAT, tm);
        return s;
    }

    const char* c_str(const char* fmt = DEFAULT_DATE_FORMAT, char* buffer = DEFAULT_DATE_BUFFER, size_t buffer_size = DEFAULT_DATE_BUFFER_LENGTH) const {
        struct tm* tm = localtime(&t);
